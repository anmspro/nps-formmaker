  ...
  <div v-if="show">
    <el-form label-position="top">
      ...
      <el-form-item :label="$t('fm.config.widget.option')"
                    v-if="Object.keys(data.options).indexOf('options')>=0 || Object.keys(data.options).indexOf('options1')>=0 || Object.keys(data.options).indexOf('options2')>=0" > 
        ...
        <template v-else>
          <template v-if="data.type=='radio' || data.type=='mcqSingle' || (data.type=='select'&&!data.options.multiple)">
            ...
            <el-radio-group v-model="data.options.defaultValue">
              ...
              >
                <li v-for="(item, index) in data.options.options" :key="index" >
                  ...
                  <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><i class="iconfont icon-icon_bars"></i></i>
                  <el-button @click="handleOptionsRemoveNPS(index)" circle plain type="danger" size="mini" icon="el-icon-minus" style="padding: 4px;margin-left: 5px;"></el-button>
          ...
          <template v-if="data.type=='matrix'">
   
            <el-radio-group v-model="data.options.defaultValue[0]">
              <draggable tag="ul" :list="data.options.options1" 
                v-bind="{group:{ name:'options'}, ghostClass: 'ghost',handle: '.drag-item'}"
                handle=".drag-item"
              >
                <li v-for="(item, index) in data.options.options1" :key="index" >
                  <el-radio
                    :label="item.value" 
                    style="margin-right: 5px;"
                  >
            ...
            <el-radio-group v-model="data.options.defaultValue[1]">
              <draggable tag="ul" :list="data.options.options2" 
                v-bind="{group:{ name:'options'}, ghostClass: 'ghost',handle: '.drag-item'}"
                handle=".drag-item"
              >
                <li v-for="(item, index) in data.options.options2" :key="index" >
                  <el-radio
                    :label="item.value" 
                    style="margin-right: 5px;"
                  >
          ...
          <template v-if="data.type=='checkbox' || data.type=='mcqMultiple' || (data.type=='select' && data.options.multiple)">
      ...
      <el-form-item :label="$t('fm.config.widget.rowTitle')" v-if="Object.keys(data.options).indexOf('rowOptions')>=0">
        <template v-if="data.type=='matrixDynamic'">
            <draggable tag="ul" :list="data.options.rowOptions" 
                v-bind="{group:{ name:'rowOptions'}, ghostClass: 'ghost',handle: '.drag-item'}"
                handle=".drag-item"
              >
            <el-radio-group v-for="(item, index) in data.options.rowOptions" v-model="data.options.rowOptions[index].name">
              
                <el-radio
                  :label="item.name" 
                  style="margin-right: 5px;"
                >
                  <el-input :style="{'width': data.options.showLabel? '90px': '180px' }" size="mini" v-model="item.name"></el-input>
                  <el-input style="width:90px;" size="mini" v-if="data.options.showLabel" v-model="item.value"></el-input>
                  <!-- <input v-model="item.value"/> -->                  
                </el-radio>
                <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><i class="iconfont icon-icon_bars"></i></i>
                <el-button @click="handleOptionsRemoveRow(index)" circle plain type="danger" size="mini" icon="el-icon-minus" style="padding: 4px;margin-left: 5px;"></el-button>
              
            </el-radio-group>
            </draggable>

            <div style="margin-left: 22px;">
            <el-button type="text" @click="handleAddRowOption">{{$t('fm.actions.addRowOption')}}</el-button>
            </div>
          </template>
      </el-form-item>

      <el-form-item :label="$t('fm.config.widget.columnTitle')" v-if="Object.keys(data.options).indexOf('columnOptions')>=0">
        <template v-if="data.type=='matrixDynamic'">
            <draggable tag="ul" :list="data.options.columnOptions" 
                v-bind="{group:{ name:'options'}, ghostClass: 'ghost',handle: '.drag-item'}"
                handle=".drag-item"
              >
            <el-radio-group v-for="(item, index) in data.options.columnOptions" v-model="data.options.columnOptions[index].name">
                ...
                <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><i class="iconfont icon-icon_bars"></i></i>
                <el-button @click="handleOptionsRemoveColumn(index)" circle plain type="danger" size="mini" icon="el-icon-minus" style="padding: 4px;margin-left: 5px;"></el-button>
              
            </el-radio-group>
            </draggable>
            <div style="margin-left: 22px;">
            <el-button type="text" @click="handleAddColumnOption">{{$t('fm.actions.addColumnOption')}}</el-button>
            </div>
          </template>
      </el-form-item>
      ...
      <el-form-item :label="$t('fm.config.widget.defaultValue')" v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'openEnded' || data.type == 'input' || data.type=='rate' || data.type=='rating' || data.type=='color' || data.type=='switch' || data.type=='text')">
        <el-input v-if="data.type=='textarea'" type="textarea" :rows="5" v-model="data.options.defaultValue"></el-input>
        <el-input v-if="data.type=='openEnded'" type="textarea" :rows="2" v-model="data.options.defaultValue"></el-input>
        ...
        <el-rate v-if="data.type == 'rating'" style="display:inline-block;vertical-align: middle;" :max="data.options.max" :allow-half="data.options.allowHalf" v-model="data.options.defaultValue"></el-rate>
        <el-button type="text" v-if="data.type == 'rating'" style="display:inline-block;vertical-align: middle;margin-left: 10px;" @click="data.options.defaultValue=0">{{$t('fm.actions.clear')}}</el-button>
        <el-color-picker 
...
export default {
  ...
  methods: {
    ...
    handleOptionsRemoveRow (index) {
      this.data.options.rowOptions.splice(index, 1)
      this.data.options.defaultValue.splice(index, 1)
    },

    handleOptionsRemoveColumn (index) {
      this.data.options.columnOptions.splice(index, 1)
    },

    handleOptionsRemoveNPS (index) {
      if (this.data.type === 'grid') {
    ...
    handleAddRowOption () {
      this.data.options.rowOptions.push({
          name: this.$t('fm.config.widget.newOption'),
          value: this.$t('fm.config.widget.newOption')
        })
      this.data.options.defaultValue.push('')
    },

    handleAddColumnOption () {
      this.data.options.columnOptions.push({
          name: this.$t('fm.config.widget.newOption'),
          value: this.$t('fm.config.widget.newOption')
        })
    },
